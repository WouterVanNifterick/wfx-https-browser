...
var
  s: string;
begin
  if (LineIndex > List.Count - 1) or (SubLineIndex > List.Count - 1) then
    Exit;

  s := '\' + List[LineIndex];
  if SubLineIndex > -1 then
    s := s + '\' + List[SubLineIndex];
...
var
  ...
  i: Integer;
begin
  for i := 0 to IconsDef.Count - 1 do
  begin
    p := Pos('=', IconsDef[i]);
    s := Copy(IconsDef[i], 1, p - 1);
    if Path.StartsWith(s) then
      Exit(Copy(IconsDef[i], p + 1, Length(IconsDef[i])));
  end;
  Result := 'MAINICON';
end;
